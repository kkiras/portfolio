...
export default function Home() {
    const iconSize = 32
    const links = [
        {id: 'github', icon: <GithubIcon size={iconSize} />, link: 'https://github.com/kkiras'},
        {id: 'upwork', icon: <UpworkIcon size={iconSize}  />, link: 'https://www.upwork.com/freelancers/~0124ef56b2424faeb9?mp_source=share'},
        {id: 'gmail', icon: <GmailIcon size={iconSize}  />, link: 'https://mail.google.com/mail/?view=cm&fs=1&to=kkiras.xd@gmail.com'},
        {id: 'discord', icon: <DiscordIcon size={iconSize} />, link: 'https://discord.com/users/1328078858376712295'}
    ]
    return (
        <div id="section-home" className={styles.container}>
            <DragableBg />

            <div className={styles.content}>
                <img className={styles.avatar} src={avatar} alt="" />

                <h2>Thanh Vinh Tran</h2>
                <p>A Software Engineer</p>

                <div className={styles.linkContainer}>
                    {links.map(link => (
                        <a key={link.id} href={link.link} id={link.id} className={styles.link} >
                            {link.icon}
                        </a>
                    ))}
                </div>
...
function LinkedInIcon({ size }) {
    return (
        <svg width={size} height={size} viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg" fill="none">
            <rect width="132" height="132" x="30" y="30" stroke="currentColor" strokeWidth="12" rx="16"/>
            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="12" d="M66 86v44"/>
            <circle cx="66" cy="64" r="8" fill="currentColor"/>
            <path stroke="currentColor" strokeLinecap="round" strokeWidth="12" d="M126 130v-26c0-9.941-8.059-18-18-18v0c-9.941 0-18 8.059-18 18v26"/>
        </svg>
    )
}

function GmailIcon({ size }) {
    return (
        <svg width={size} height={size} viewBox="0 0 192 192" xmlns="http://www.w3.org/2000/svg" fill="none">
